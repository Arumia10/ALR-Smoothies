...
export function initializeOrderLogic() {
  ...
  document.getElementById('to-payment')?.addEventListener('click', () => {
    ...
    if (rl === 'Student') {
      const day = new Date(pd).getUTCDay();
      if (day !== 3) {
        alert("Students may only select Wednesday.");
        return;
      }
    }
  ...
  document.getElementById('confirm-pay')?.addEventListener('click', async () => {
    ...
    if (role === 'Student') {
      ...
      deliveryLocation = `Pickup in front of the library at 12:55`;
    } else {
      if (wd === 3) {
        ...
        alert("Please choose a valid pickup day.");
        return;
      }
    }
    ...
    try {
      ...
      const returnInstructions =
        role === "Student"
          ? `
            🔁 Please return the smoothie glasses in the designated return area.
          `
          : `
